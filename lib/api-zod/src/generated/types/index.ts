/**
 * Generated by orval v8.9.1 🍺
 * Do not edit manually.
 * Api
 * AutoServis API
 * OpenAPI spec version: 0.1.0
 */

export * from './appointment';
export * from './appointmentInput';
export * from './appointmentInputStatus';
export * from './appointmentStatus';
export * from './appointmentUpdate';
export * from './appointmentUpdateStatus';
export * from './aresResult';
export * from './auditLogEntry';
export * from './authStatus';
export * from './authStatusRole';
export * from './backup';
export * from './backupRunResult';
export * from './changePasswordInput';
export * from './changeScannerPasswordInput';
export * from './checkLoanerOverlapParams';
export * from './consentHistoryEntry';
export * from './consentHistoryEntryBasis';
export * from './consentHistoryEntryEvent';
export * from './customerReminderLogEntry';
export * from './dashboardSummary';
export * from './error';
export * from './forgotPasswordInput';
export * from './gdprActionResult';
export * from './gdprExport';
export * from './gdprSearchParams';
export * from './gdprSearchResult';
export * from './gdprVehicleMatch';
export * from './gdprVehicleMatchLegalBasis';
export * from './getAuditLogParams';
export * from './getRetentionReportParams';
export * from './healthStatus';
export * from './healthStatusDatabase';
export * from './healthStatusStorage';
export * from './importMaterialsInput';
export * from './importMaterialsInputItemsItem';
export * from './importTpInput';
export * from './importTpResult';
export * from './invoiceImportInput';
export * from './invoiceImportInputPdfsItem';
export * from './invoiceImportResult';
export * from './invoiceImportResultItemsItem';
export * from './listAppointmentsParams';
export * from './listAuditLogParams';
export * from './listLoanerCustomerSuggestionsParams';
export * from './listLoanersParams';
export * from './listLoanersStatus';
export * from './listMaterialsParams';
export * from './listVehicleModelsParams';
export * from './listVehiclesParams';
export * from './listWorkOrdersParams';
export * from './listWorkOrdersStatus';
export * from './loaner';
export * from './loanerCustomerSuggestion';
export * from './loanerInput';
export * from './loanerInputStatus';
export * from './loanerStatus';
export * from './loanerUpdate';
export * from './loanerUpdateStatus';
export * from './loginInput';
export * from './materialCatalogInput';
export * from './materialCatalogItem';
export * from './materialCatalogUpdate';
export * from './materialQrPayload';
export * from './materialsImportResult';
export * from './messageResult';
export * from './missingObject';
export * from './photo';
export * from './resetPasswordInput';
export * from './restoreTrashItemInput';
export * from './retentionCategory';
export * from './retentionItem';
export * from './retentionReport';
export * from './scanMaterialsInput';
export * from './scanMaterialsResult';
export * from './scanMaterialsResultSuggestionsItem';
export * from './scanMaterialsResultSuggestionsItemSource';
export * from './serviceRecord';
export * from './serviceRecordInput';
export * from './setConsentInput';
export * from './setConsentInputLegalBasis';
export * from './setScannerPasswordInput';
export * from './settings';
export * from './settingsInput';
export * from './storageCleanupRequest';
export * from './storageCleanupResult';
export * from './storageIntegrityResult';
export * from './successResult';
export * from './testReminderResult';
export * from './trashItem';
export * from './vehicle';
export * from './vehicleDetail';
export * from './vehicleDetailLegalBasis';
export * from './vehicleDetailOwnerType';
export * from './vehicleDetailTransmission';
export * from './vehicleInput';
export * from './vehicleInputOwnerType';
export * from './vehicleInputTransmission';
export * from './vehicleLegalBasis';
export * from './vehicleOwnerType';
export * from './vehicleTransmission';
export * from './vehicleUpdate';
export * from './vehicleUpdateOwnerType';
export * from './vehicleUpdateTransmission';
export * from './workOrder';
export * from './workOrderInput';
export * from './workOrderInputInvoiceStatus';
export * from './workOrderInputPaymentStatus';
export * from './workOrderInvoiceStatus';
export * from './workOrderMaterial';
export * from './workOrderMaterialInput';
export * from './workOrderMaterialUpdateInput';
export * from './workOrderPaymentStatus';
export * from './workOrderStatus';
export * from './workOrderUpdate';
export * from './workOrderUpdateInvoiceStatus';
export * from './workOrderUpdatePaymentStatus';
export * from './workOrderUpdateStatus';
