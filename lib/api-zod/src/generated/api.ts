...
export const GetVehicleResponse = zod.object({
  ...
  "model": zod.string().nullish(),
  "ownerName": zod.string().nullish(),
  "status": zod.enum(['open', 'in_progress', 'waiting_parts', 'needs_return', 'completed']).describe('Work status'),
  "invoiceStatus": zod.enum(['not_invoiced', 'ready_to_invoice', 'invoiced']).describe('Invoicing status'),
  "paymentStatus": zod.enum(['unpaid', 'partial', 'paid']).describe('Payment status'),
  "km": zod.number().nullish(),
  "description": zod.string().nullish(),
...
export const CreateWorkOrderBody = zod.object({
  "licensePlate": zod.string().min(1),
  "invoiceStatus": zod.enum(['not_invoiced', 'ready_to_invoice', 'invoiced']).optional(),
...
export const GetDashboardSummaryResponse = zod.object({
  ...
  "orderedToday": zod.number().optional().describe('Work orders created today (Dnes objednáno)'),
  "waitingParts": zod.number().optional().describe('Work orders waiting for parts (Čeká na díly)'),
  "readyToInvoice": zod.number().optional().describe('Work orders ready to invoice (Hotovo k fakturaci)'),
  "invoicedUnpaid": zod.number().optional().describe('Invoiced work orders not fully paid (Vyfakturováno-nezaplaceno)'),
  "stkOverdue": zod.number().optional().describe('Vehicles with STK already expired (STK po termínu)'),
