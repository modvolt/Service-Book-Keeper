...
- AI env: `AI_INTEGRATIONS_OPENAI_BASE_URL` + `AI_INTEGRATIONS_OPENAI_API_KEY` (provided by the Replit OpenAI integration in dev; set manually in prod, e.g. base URL `https://api.openai.com/v1`). `OPENAI_MODEL` selects the chat model (default `gpt-5.4`). The vehicle scan (Načtení vozu) uses a single in-code prompt (`SYSTEM_PROMPT` in `vehicle-import.ts`) sent via the **Responses API** over `AI_INTEGRATIONS_OPENAI_*` — there is no platform-managed/stored prompt. (OpenAI is retiring stored prompts; the prior `OPENAI_PROMPT_ID` / `OPENAI_API_KEY` / `OPENAI_PROMPT_VERSION` / `OPENAI_BASE_URL` stored-prompt path was removed.)
