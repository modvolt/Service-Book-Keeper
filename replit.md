...
- Stored-prompt env (optional, for the vehicle scan): `OPENAI_PROMPT_ID` (a `pmpt_…` prompt managed on the OpenAI platform) switches Načtení vozu to the Responses API with that prompt, calling OpenAI **directly** via `OPENAI_API_KEY` (+ optional `OPENAI_BASE_URL`, default `https://api.openai.com/v1`; optional `OPENAI_PROMPT_VERSION` to pin a version) — no Replit AI integration involved. Unset = keep the built-in inline prompt over `AI_INTEGRATIONS_OPENAI_*`
