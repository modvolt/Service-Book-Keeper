...
- Auth/session env: `APP_PASSWORD` (admin login password, bootstraps the admin auth row), `SCANNER_PASSWORD` (optional; seeds the restricted "scanner" role auth row on first login — leave unset to keep the scanner account disabled; once an admin disables it in the UI this var will not silently re-enable it), `SESSION_SECRET` (signing key; fail-fast in production), `ALLOWED_ORIGINS` (comma-separated CORS allowlist; reflected in dev)
...
- AI env (canonical set, exactly three variables): `AI_INTEGRATIONS_OPENAI_BASE_URL` + `AI_INTEGRATIONS_OPENAI_API_KEY` (provided by the Replit OpenAI integration in dev; set manually in prod, e.g. base URL `https://api.openai.com/v1`) and `OPENAI_MODEL` (chat model, default `gpt-5.5`). Both the vehicle scan (Načtení vozu, `SYSTEM_PROMPT` in `vehicle-import.ts`) and the material/invoice extraction use a single in-code prompt sent via the **Responses API** over `AI_INTEGRATIONS_OPENAI_*` — there is no platform-managed/stored prompt. The dead stored-prompt variables (`OPENAI_PROMPT_ID` / `OPENAI_API_KEY` / `OPENAI_PROMPT_VERSION` / `OPENAI_BASE_URL`) were never wired into code and have been removed from `.env.example`.
- Diagnostics env (optional): `ENABLE_PUBLIC_DIAGNOSTICS` — the diagnostics page (`/api/diagnostics`) and its error feed (`/api/diagnostics/errors`) are **admin-only by default**; set this to `true`/`1`/`yes`/`on` to expose them without a login (a short debugging window only). Captured errors are scrubbed of credential-looking strings (connection-string passwords, bearer tokens, API keys) before they are stored, so diagnostics never prints secrets.
- Backup env (optional): `BACKUP_RETENTION_COUNT` — number of most-recent automatic backups to keep (default 14; older ones are pruned with their blobs)
- Trash env (optional): `TRASH_RETENTION_DAYS` — soft-deleted items older than this many days are auto-purged (default 30; reuses the manual purge path so photo blobs are freed before DB rows)
...
- Work orders: create with SPZ entry (auto-resolves vehicle), service item checkboxes, and three independent state machines tracked separately: work `status` (`open` / `in_progress` / `waiting_parts` / `needs_return` / `completed`), `invoiceStatus` (`not_invoiced` / `ready_to_invoice` / `invoiced`), and `paymentStatus` (`unpaid` / `partial` / `paid`). The old single `paid` boolean (Zaplaceno) was replaced by this split (migrations `0008`/`0009`); all three are shown as badges in the list and edited on the detail page. Setting `invoiceStatus=invoiced` auto-closes any open loaner for the order.
...
- Automatic backups (Nastavení): toggle (`backupsEnabled`) for a daily gzipped JSON snapshot of all business tables, uploaded to the object store under `backups/` via the storage facade (`putBackupObject`). Same snapshot shape as the manual `/backup/export`, so a downloaded backup restores through the existing "Obnovit ze zálohy" import. Tracked in the `backups` table (driver has no list API, so retention is DB-driven); `runBackup()` prunes beyond `BACKUP_RETENTION_COUNT` (deletes blob then row). Scheduler runs on the hourly tick with a once-per-calendar-day guard via `settings.lastBackupAt` (mirrors the reminder pattern; a failed run leaves `lastBackupAt` untouched so it retries). UI: enable toggle, "Spustit zálohu nyní" manual run, and a recent-backups list with per-item download (download decompresses server-side to plain JSON via a plain authed fetch, not codegen).
- GDPR (`/gdpr`): search personal data (by name/phone/email/SPZ), per-vehicle export (JSON download), anonymize (strip owner + appointment PII, keep technical history), permanent delete (vehicle + work orders + appointments + service records + photo blobs), and an audit-log viewer. Lawful-basis tracking: each vehicle has a `legalBasis` (`contract` / `legitimate_interest` / `consent`) plus an append-only `consent_history` table recording every `granted` / `withdrawn` / `updated` / `migrated` event (basis, note, actor, timestamp). Migration `0010` adds `vehicles.legal_basis` + `consent_history` and backfills a `consent` basis + seed history row for any vehicle that already had `consentGivenAt`.
- Trash / Koš (`/trash`): soft-delete-and-restore for `vehicle`, `work_order`, `service_record`, `material`, `loaner`, `appointment`, and `photo`. Deleting sets `deletedAt` (+ `deletedBy` / `deleteReason`) instead of removing the row, and EVERY read/get/update/FK-resolve/scan query filters on `isNull(deletedAt)` so deleted rows never leak into lists, the dashboard, or scan resolution. `GET /trash` lists soft-deleted items; `POST /trash/:entity/:id/restore` clears the flag; `DELETE /trash/:entity/:id` hard-deletes (purge) — for photos/work orders it deletes the storage blob BEFORE the DB row (erasure ordering) and aborts the purge if the blob delete fails. The shared purge logic (`purgeTrashItem`) is also driven by an automatic retention sweep: items whose `deletedAt` is older than `TRASH_RETENTION_DAYS` (default 30) are permanently purged under the `system` actor on the hourly scheduler tick, guarded once-per-calendar-day via `settings.lastTrashCleanupAt` (mirrors the backup scheduler). Per-item failures (e.g. a blob delete) are isolated — the item stays in the trash and is retried next run.
- Audit log: the `audit_log` table records sensitive actions (`actor` = `admin` / `scanner` / `system`) with a sanitized PII-free `snapshot`. Covers auth (`login` / `login_failed` / `logout` / `password_changed` / `password_reset*` / `scanner_password_changed` / `scanner_password_deleted`), GDPR (`gdpr_export` / `gdpr_anonymize` / `gdpr_delete` / `gdpr_consent`), entity lifecycle (`entity_created` / `entity_updated` / `entity_deleted` / `entity_restored` / `entity_purged`), and `storage_orphans_cleaned`. Action names live in `lib/audit-actions`.
- Roles: two login roles share one login form. `admin` (`APP_PASSWORD`) has full access; `scanner` (`SCANNER_PASSWORD`) is restricted to the mobile material/document scanner. The scanner router mounts before the admin gate and admins also pass the scanner gate, so dual-role paths use one role-branching handler; scanner writes bind to the scanned SPZ, never to a token. An admin can disable the scanner from the UI (a `__disabled__` sentinel hash) so `SCANNER_PASSWORD` can't silently re-seed it.
